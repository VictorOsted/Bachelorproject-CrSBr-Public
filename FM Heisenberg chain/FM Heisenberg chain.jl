...
save(joinpath(@__DIR__, "FM Heisenberg chain crystal.png"), view_crystal(cryst; ndims=3, ghost_radius=3))
...
J = -1 # Sets the exchange constant to be negative which means it is ferromagnetic
...
save(joinpath(@__DIR__, "FM Heisenberg chain spins.png"),plot_spins(sys; ndims=2, ghost_radius=3)) # Saves a png of the spin configuration

# Calculates the spin wave spectrum and saves it as a png
# Saves the data as a .txt file
function save_int(sys; fpath=joinpath(@__DIR__, "FM Heisenberg chain intensities.txt"),size=[500,1000], Elim=[0,10], Eres=0.3, qs=[[0,0,0],[1,0,0]])
    ...
    save(joinpath(@__DIR__, "FM Heisenberg chain intensities.png"), plot_intensities(res; units=units))
...
function save_int_energies(sys; fpath=joinpath(@__DIR__, "FM Heisenberg chain energies.txt"),size=[500,1000], Elim=[0,10], Eres=0.3, qs=[[0,0,0],[1,0,0]])
    # Save .txt file with intensities of spinwave colorplot
    swt = SpinWaveTheory(sys; measure=ssf_perp(sys))
    path = q_space_path(cryst, qs, size[1])
